...
spGAM_count <- function(df, printplot = FALSE, saveplot = FALSE) {
  ...
  result <- try({
    ...
    g <- df_n %>%
      ...
      geom_ribbon(aes(ymax = yucl, ymin = ylcl),
                  fill = grey(0.5),
                  alpha = 0.4) +
      ggtitle(paste("gam_count", ptitle))
...
spGAM_count_ns <- function(df, printplot = FALSE, saveplot = FALSE) {
  ...
  result <- try({
    ...
    g <- df_n %>%
      ...
      geom_ribbon(aes(ymax = yucl, ymin = ylcl),
                  fill = grey(0.5),
                  alpha = 0.4) +
      ggtitle(paste("gam_count_ns", ptitle))
